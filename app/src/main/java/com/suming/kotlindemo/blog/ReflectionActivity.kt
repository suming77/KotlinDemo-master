...
class ReflectionActivity : AppCompatActivity() {
    ...
    fun classQuote() {
        //Kotlin 类引用
        val reflection: KClass<ReflectionActivity> = ReflectionActivity::class//ReflectionActivity类的引用
        //引用 quo 是 KClass 类型的值， KClass 表示一个类，并提供自检功能。

        //java 类引用
        val reflectionJava: Class<ReflectionActivity> = ReflectionActivity::class.java
...
fun ReflectionActivity.Person.isChild() = age > 18
